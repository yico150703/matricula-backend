...
with app.app_context():
    ...
    if db.session.get(Alumno, args.codigo) or Alumno.query.filter_by(email=args.email.lower()).first():
        raise SystemExit("Ya existe un alumno con ese código o email.")
    db.session.add(Alumno(cod_alumno=args.codigo, email=args.email.lower(), password_hash=generate_password_hash(args.password), nombres=args.nombres, apellidos=args.apellidos, id_plan=args.plan, estado="activo", fecha_ingreso=date.fromisoformat(args.fecha_ingreso)))
    db.session.commit()
